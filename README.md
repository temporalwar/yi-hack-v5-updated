...
# Install CMake 3.25+ — mosquitto 2.1.x requires CMake 3.18 minimum
wget -q https://github.com/Kitware/CMake/releases/download/v3.25.3/cmake-3.25.3-linux-x86_64.sh \
     -O /tmp/cmake.sh && chmod +x /tmp/cmake.sh && sudo /tmp/cmake.sh --skip-license --prefix=/usr/local
...
* **CMake requirement**: mosquitto 2.1.x requires CMake 3.18+. The Dockerfile installs CMake 3.25.3 directly from Kitware. Native builds must also use CMake 3.18 or newer.
* **ONVIF**: `onvif_simple_server` binary ships in `www/onvif/` — `system.sh` automatically copies it to `bin/` on first boot if missing from PATH.
