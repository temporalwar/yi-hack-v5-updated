...
All bundled binaries are cross-compiled for `arm-hisiv300-linux` (ARMv5te, uClibc).
Versions are chosen as the **newest release that builds reliably** for this toolchain.

| Package | Updated to | Why this version |
| :--- | :--- | :--- |
| **OpenSSL** | **3.3.7** | Replaces EOL 1.1.x. Patches CVE-2025-9230, CVE-2025-9231 (fixed in 3.3.5), CVE-2025-15467, CVE-2025-69421 (3.3.6), and CVE-2026-28387/28388/28389/28390/31790 (3.3.7). |
| **curl** | **8.20.0** | Replaces an old dev snapshot; current stable with years of upstream security fixes. |
| **dropbear** | **2025.89** | Replaces 2018-era build. Patches CVE-2025-14282 — in multi-user mode the SSH server performed socket forwardings as root before dropping to the logged-in user (fixed in 2025.88). |
| **cJSON** | **1.7.18** | Lightweight JSON library used by firmware tooling. |
| **mosquitto** | **1.6.15** | Pinned to the **pure-C 1.6.x line**. Mosquitto 2.x failed to link against uClibc and has a heavier footprint; 1.6.15 links cleanly with `WITH_MEMORY_TRACKING=no` and keeps memory low. |
| **pure-ftpd** | **1.0.54** | Current stable; multiple hardening fixes over the bundled build. |
| **libfuse3** | **3.16.2** | Highest libfuse that builds with the Meson 0.51.x-era toolchain in the build image; 3.18.x requires a newer Meson than this cross-environment supports. |

> CVE references verified against the MITRE CVE database; each listed CVE is fixed in the
> pinned version or earlier. Originally the project targeted Mosquitto 2.x and a newer
> libfuse, but those do not cross-compile cleanly for ARMv5te/uClibc with this toolchain.
