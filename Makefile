...
CJSON_URL      := https://github.com/DaveGamble/cJSON/archive/v$(CJSON_VER).tar.gz
...
$(STAGING_LIB)/libssl.so.3:
	...
	cd $(OPENSSL_SRC) && ./Configure linux-armv4 shared --prefix=$(STAGING_DIR) --openssldir=$(STAGING_DIR)/ssl
...
$(STAGING_SBIN)/dropbear:
	...
	$(MAKE) -C $(DROPBEAR_SRC) -j$(shell nproc) MULTI=1 PROGRAMSS="dropbear dbclient dropbearkey scp"
	$(MAKE) -C $(DROPBEAR_SRC) install MULTI=1 PROGRAMSS="dropbear dbclient dropbearkey scp"
...
$(STAGING_LIB)/libcjson.so:
	@if [ ! -f $(DOWNLOAD_DIR)/cJSON-$(CJSON_VER).tar.gz ]; then echo " DL cJSON-$(CJSON_VER).tar.gz"; wget -q --show-progress -P $(DOWNLOAD_DIR) $(CJSON_URL); fi
	@if [ ! -d $(CJSON_SRC) ]; then tar -xzf $(DOWNLOAD_DIR)/cJSON-$(CJSON_VER).tar.gz -C $(BUILD_DIR); fi
...
$(STAGING_SBIN)/mosquitto: openssl
	@if [ ! -f $(DOWNLOAD_DIR)/mosquitto-$(MOSQUITTO_VER).tar.gz ]; then echo " DL mosquitto-$(MOSQUITTO_VER).tar.gz"; wget -q --show-progress -P $(DOWNLOAD_DIR) $(MOSQUITTO_URL); fi
	@if [ ! -d $(MOSQUITTO_SRC) ]; then tar -xzf $(DOWNLOAD_DIR)/mosquitto-$(MOSQUITTO_VER).tar.gz -C $(BUILD_DIR); fi
...
$(STAGING_LIB)/libfuse3.so.3:
	...
	@if [ ! -d $(LIBFUSE_SRC) ]; then tar -xJf $(DOWNLOAD_DIR)/fuse-$(LIBFUSE_VER).tar.xz -C $(BUILD_DIR); fi
	@rm -rf $(LIBFUSE_BUILD)
	@mkdir -p $(LIBFUSE_BUILD)
	cd $(LIBFUSE_BUILD) && meson setup $(LIBFUSE_SRC) --prefix=$(STAGING_DIR) --buildtype=minsize -Dexamples=false -Dtests=false -Dutils=false
