...
$(STAGING_LIB)/libcjson.a:
	...
	cd $(CJSON_BUILD) && cmake $(CJSON_SRC) -DCMAKE_TOOLCHAIN_FILE=$(CURDIR)/scripts/hisiv300.cmake -DCMAKE_INSTALL_PREFIX=$(STAGING_DIR) -DCMAKE_BUILD_TYPE=MinSizeRel -DENABLE_CJSON_TEST=OFF -DENABLE_CJSON_UTILS=OFF -DBUILD_SHARED_LIBS=OFF -DBUILD_STATIC_LIBS=ON
...
$(STAGING_LIB)/libmosquitto.so.1:
	...
	cd $(MOSQUITTO_BUILD) && cmake $(MOSQUITTO_SRC) -DCMAKE_TOOLCHAIN_FILE=$(CURDIR)/scripts/hisiv300.cmake -DCMAKE_INSTALL_PREFIX=$(STAGING_DIR) -DCMAKE_BUILD_TYPE=MinSizeRel -DOPENSSL_ROOT_DIR=$(STAGING_DIR) -DCJSON_INCLUDE_DIRS=$(STAGING_INC) -DCJSON_LIBRARIES=$(STAGING_LIB)/libcjson.a -DWITH_BROKER=OFF -DWITH_APPS=ON -DWITH_PLUGINS=OFF -DWITH_TLS=ON -DWITH_TLS_PSK=OFF -DWITH_THREADING=ON -DDOCUMENTATION=OFF -DWITH_STATIC_LIBRARIES=OFF -DWITH_SHARED_LIBRARIES=ON
