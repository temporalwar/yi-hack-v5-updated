...
$(STAGING_SBIN)/mosquitto: openssl
	...
	$(MAKE) -C $(MOSQUITTO_SRC) -j$(shell nproc) CC=$(CC) CXX=$(CXX) WITH_TLS=yes WITH_TLS_PSK=yes WITH_THREADING=no WITH_DOCS=no CFLAGS="$(COMMON_CFLAGS) -I$(STAGING_INC) -std=gnu99 -D_GNU_SOURCE" CXXFLAGS="$(COMMON_CFLAGS) -I$(STAGING_INC)" LDFLAGS="$(COMMON_LDFLAGS) -L$(STAGING_LIB)"
